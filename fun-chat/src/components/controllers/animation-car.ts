import type { ISuccessRaceCars } from '../types/types';
...
async function startFunctionality(element: HTMLElement): Promise<void> {
  const id = Number(element.id.split('start-car-').pop());
  const currentCar = document.querySelector<HTMLElement>(`#car-${id}`);
...
async function stopFunctionality(element: HTMLElement): Promise<void> {
  const id = Number(element.id.split('stop-car-').pop());
  const currentCar = document.querySelector<HTMLElement>(`#car-${id}`);
...
async function raceFunctionality(
  element: HTMLElement,
  numberPage: number,
  winnerMessage: HTMLElement | null
): Promise<void> {
  disableAllControlButton(element);
  const carsArrayResult = await CarServices.getCars(numberPage);
  const arrayCars = carsArrayResult?.item;
  const promisesStart = arrayCars?.map(async element_ => {
    const currentCarStart = document.querySelector<HTMLElement>(`#car-${element_.id}`);
...
async function resetFunctionality(
  element: HTMLElement,
  numberPage: number,
  winnerMessage: HTMLElement | null
): Promise<void> {
  try {
    ...
    const arrayCars = carArrayResult?.item;
    const promisesStop = arrayCars?.map(async element_ => {
      const currentCarStop = document.querySelector<HTMLElement>(`#car-${element_.id}`);
      ...
      (async (): Promise<void> => {
        switch (true) {
          case element.classList.contains('start-icon'): {
            await startFunctionality(element);
            break;
          }

          case element.classList.contains('stop-icon'): {
            await stopFunctionality(element);
            break;
          }

          case element.classList.contains('race'): {
            await raceFunctionality(element, numberPage, winnerMessage);
            break;
          }

          case element.classList.contains('reset'): {
            await resetFunctionality(element, numberPage, winnerMessage);
            break;
          }
